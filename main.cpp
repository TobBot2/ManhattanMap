...
using LightCount = LightIndex;
using Color = vmath_hpp::vec<uint8_t, 3>;
...
LightIndex debugLight = 0;
uint32_t debugColor = 0xff0000;

static PIOHandle pioHandle;
static uint32_t pixels[TOTAL_LIGHTS_COUNT] = { 0 };

// DisplayMode mode = TrainLineColors;
...
void debug_next_light_cb(uint gpio, uint32_t events) {
    printf(".");
...
void next_display_mode_cb(uint gpio, uint32_t events) {
    switch (mode) {
        case Solid: {
...
void setup_lights(std::array<Light, TOTAL_LIGHTS_COUNT>& lights) {
    // TODO

    // register all lights
    Light::setStripPositions(lights, 0, 60, {7.f, .2f}, {7.f, 24.f});

    // intersections
    lights[12].setIsIntersection();
}

void setup_train_lines(std::array<TrainLine, TRAIN_LINES_COUNT>& trainLines) {
